...
// import { I_Project } from '@models/project.model'
...
const databaseService = {
	...
	// update: async (filterQuery: any, updateQuery: any, model: Model<I_Project>) => {
	// 	const data = await model.findOneAndUpdate(filterQuery, updateQuery, { new: true })
	// 	return data
	// },
	...
	generatePopulateArray: (pathArray: Array<string>, selectArray: Array<string>) => {
		if (!pathArray || !selectArray || pathArray.length !== selectArray.length) {
			throw new Error(App.Message.Error.PopulateError())
		}
		const populateArray = []
